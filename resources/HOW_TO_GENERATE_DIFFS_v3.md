...
### Rule 1 — Minimum 2 non-blank context lines on each side (with exact blank line preservation)

Every hunk must have at least **2 non-blank context lines before the change AND 2 non-blank context lines after the change.** (Blank lines include lines with only spaces). While blank lines do not count toward this 2-line minimum, you **MUST include any blank lines that fall between the non-blank lines** to ensure perfect context consistency and structural matching.

**Why:** Single-line anchors like `})`, `}`, `return null`, and `catch` blocks appear dozens of times in a file. The fuzzy engine will find multiple candidates and reject the hunk as ambiguous. Two non-blank lines on each side gives the matcher a unique enough fingerprint, and preserving the intermediate blank lines ensures it matches the exact source text.

**BOUNDARY EXCEPTIONS:**
You must ignore this rule **IF AND ONLY IF** the diff is editing code where there are not 2 lines before (if it is editing one of the first 2 lines of code in the file) or not 2 lines after (if it is editing one of the last 2 lines of code in the file). 

If you are applying this exception, you **must inform the user** by preceding the generated diff hunk with an H1 header specifying the boundary constraint:
- `# This hunk is from the beginning of the codebase`
- `# This hunk is from the end of the codebase`
...
FOR EACH file requiring changes:
  ...
  FOR EACH logical change in that file:
    LOCATE anchor window {
      FIND 2+ non-blank lines BEFORE the change (include all interstitial blank lines)
      FIND 2+ non-blank lines AFTER the change (include all interstitial blank lines)
      
      IF near beginning of file (fewer than 2 lines before exist):
        USE available lines
        PREFIX diff block with `# This hunk is from the beginning of the codebase`
        
      IF near end of file (fewer than 2 lines after exist):
        USE available lines
        PREFIX diff block with `# This hunk is from the end of the codebase`
        
      IF uniqueness is uncertain: EXPAND to 3+ lines
    }

  GENERATE_PATCH(strict_unified_diff_format) {
    BOUNDARY_HEADER: IF applicable, output the H1 boundary warning before the fences
    ...
    CONTEXT_BEFORE: 2+ non-blank lines (plus blank lines between them), single space prefix, raw code only
    REMOVALS:       `-` prefix
    ADDITIONS:      `+` prefix
    CONTEXT_AFTER:  2+ non-blank lines (plus blank lines between them), single space prefix, raw code only
  }

  AUDIT {
    ASSERT(context lines stripped of N| prefix)
    ASSERT(2+ non-blank context lines on each side of every hunk UNLESS boundary exception applies)
    ASSERT(All interstitial blank lines are preserved within context windows)
...
### ✅ CORRECT — Standard edit with 2 non-blank context lines and intermediate blank lines included
...
+
         ...
         <button className="sidebar-back-btn" onClick={() => { setProject(null); setProjectFilePath(null) }}>
           ← Projects
```
*(Notice the blank lines preserved within the context window to ensure exact mapping)*
...
### ✅ CORRECT — Exception Example: Beginning of File
...
### ❌ Fewer than 2 non-blank context lines on one side (without boundary exception)
```diff
@@ -98,4 +98,5 @@
   })
+  const newThing = true
...
```
Only one non-blank context line before and after. The patcher will reject this at load time unless it explicitly is the beginning/end of the file and carries the correct H1 declaration.

### ❌ Omitting blank lines in context
```diff
@@ -120,4 +120,5 @@
   const init = () => {
   }
+  init();
   export default init;
```
If there was a blank line between `}` and `export default init;` in the file, omitting it here breaks the context match. Include it.
...
| Error | Meaning | Fix |
|---|---|---|
| `context not found in file` | The anchor lines don't exist anywhere in the file (or blank lines were omitted) | Verify context lines match the matrix exactly, including blank lines |
...
| `Hunk rejected at load: fewer than 2 non-blank context lines` | Diff rejected before staging | Add more context lines around the change, unless a valid boundary exception |
...
- [ ] 2+ non-blank context lines before every change (or valid boundary exception H1 header)
- [ ] 2+ non-blank context lines after every change (or valid boundary exception H1 header)
- [ ] ALL blank lines between non-blank context lines are included to ensure exact layout matching
